...
export function setupMirageServer() {
  return createServer({
    ...
    factories: {
      host: Factory.extend({
        name(i) { return `Host ${i + 1}`; },
        mac() {
          return Array.from({length: 6}, () =>
            Math.floor(Math.random() * 256).toString(16).padStart(2, '0').toUpperCase()
          ).join(':');
        },
        ip(i) { return `192.168.1.${100 + i}`; },
        autoWake() { return Math.random() > 0.5; },
        status() { return Math.random() > 0.3; }
      })
    },
    ...
    routes() {
      ...
      this.post('/hosts', (schema, request) => {
        ...
        const existingHost = schema.hosts.all().models.find(h =>
          h.mac === hostData.mac || h.ip === hostData.ip
        );
